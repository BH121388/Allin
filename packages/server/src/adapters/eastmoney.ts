...
export async function fetchAllFunds(): Promise<FundInfo[]> {
  ...
  if (!response || !response.ok) {
    console.warn('[eastmoney] 基金列表 API 请求失败');
    return [];
  }
  ...
  try {
    ...
    if (!rows || rows.length === 0) {
      console.warn('[eastmoney] 基金列表解析失败');
      return [];
    }
    ...
    console.warn('[eastmoney] 基金列表处理异常:', err);
    return [];
  }
}
...
export async function fetchFundNAV(code: string, days = 90): Promise<NAVEntry[]> {
  ...
  // 使用 pingzhongdata API（已验证可用，返回完整历史净值）
  const url = `http://fund.eastmoney.com/pingzhongdata/${code}.js`;
  const response = await fetchWithTimeout(url);

  if (!response || !response.ok) {
    console.warn(`[eastmoney] 基金 ${code} pingzhongdata API 请求失败`);
    return [];
  }

  try {
    const text = await response.text();
    // 提取单位净值走势: var Data_netWorthTrend = [...]
    const trendMatch = text.match(/var Data_netWorthTrend\s*=\s*(\[[\s\S]*?\]);/);
    if (!trendMatch) {
      console.warn(`[eastmoney] 基金 ${code} 净值数据解析失败`);
      return [];
    }

    const data: Array<{ x: number; y: number; equityReturn: number }> = JSON.parse(trendMatch[1]);
    if (!Array.isArray(data) || data.length === 0) return [];

    const entries: NAVEntry[] = data.map((item, i) => {
      const date = new Date(item.x);
      const dateStr = `${date.getFullYear()}-${String(date.getMonth()+1).padStart(2,'0')}-${String(date.getDate()).padStart(2,'0')}`;
      return {
        date: dateStr,
        nav: parseFloat(item.y.toFixed(4)),
        accNav: parseFloat(item.y.toFixed(4)), // pingzhongdata 只返回单位净值
        dailyReturn: parseFloat((item.equityReturn || 0).toFixed(4)),
      };
    });

    // 只取最近 days 条
    const result = entries.slice(-days);
    console.log(`[eastmoney] 成功获取基金 ${code} ${result.length} 条净值（共${entries.length}条历史数据）`);
    return result;
  } catch (err) {
    console.warn(`[eastmoney] 基金 ${code} 净值处理异常:`, err);
    return [];
  }
}

...
## 接下来要做什么（2026-05-20 更新）

### ⚠️ Fix 1: 重仓股名称查找表覆盖中小盘
- **根因**：代码清洗（cleanStockCode）本身正确，但 STOCK_NAME_MAP 只有 180 只大盘蓝筹股
- **案例**：008983 持仓 = 新易盛(300502)、源杰科技(688498)、中际旭创(300308)等 — 全部不在表中
- **方案**：扩展查找表到 500+只，覆盖中小盘热门赛道；查不到的尝试从 API 获取
- **测试基准**：008983 的 10 只持仓名称必须全部正确显示

### ⚠️ Fix 2: 持仓净值使用真实数据
- **根因**：持仓页面用 `getMockNAV()` 返回随机 mock 净值，而不是真实净值
- **案例**：用户买 008983 成本 2.0847，今日 2.0988，系统却显示 2.8677
- **方案**：`GET /api/portfolio` 改用 `fetchFundDetail(code)` 获取 pingzhongdata 真实净值
- **验证**：买入任意基金后，显示的当前净值应匹配天天基金官网

### ⚠️ Fix 3: 推荐+分析策略转向中小盘热门
- **根因**：mock 10 只基金全是蓝筹消费/金融/50ETF 等大盘风格，股票模板也是茅台腾讯
- **要求**：排除茅台/腾讯/阿里/小米等超大市值，转向中大盘以下、当下热门赛道
- **方案**：
  1. mock 基金池替换为中小盘/科技/高端制造/AI/机器人/低空经济等热门赛道
  2. 股票模板替换为中小盘成长股
  3. 评分权重适度提升行业景气匹配(sectorMatch)权重，降低规模流动性(scale)权重
- **测试基准**：推荐 Top 5 不应包含茅台/腾讯/工行等超大市值重仓的基金
